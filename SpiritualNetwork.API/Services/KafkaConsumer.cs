...
namespace SpiritualNetwork.API.Services
{
	...
	{
		private readonly List<string> _topics = new List<string> { "post", "hashtag" };
		...
		protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			//{
			//	BootstrapServers = "4.240.89.219:9092",
                    ...
                    try
                    {
                        while (!stoppingToken.IsCancellationRequested)
                        {
                            ...
                            using (var scope = _serviceScopeFactory.CreateScope())
                            {
                                ...
                                if (messageObject.Topic == "hashtag")
                                {
                                    consumer.Commit(consumeResult);
                                    int postId = messageObject.PostId;
                                    //var postDataDto = JsonConvert.DeserializeObject<UploadPostResponse>(consumeResult.Message.Value);
                                    var hashTagService = scope.ServiceProvider.GetRequiredService<IHastTagService>();
                                    var result = await hashTagService.ExtractPostHashTag(postId);
                                    if (result != null && result.Success)
                                    {
                                        consumer.Commit(consumeResult);
                                    }
